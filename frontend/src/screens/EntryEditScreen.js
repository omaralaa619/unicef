...
const EntryEditScreen = () => {
  ...
  useEffect(() => {
    const fetchEntry = async () => {
      ...
      setinstitutionAdress(data.institutionAdress);
      setgeographicAreaCovered(data.geographicAreaCovered);
      setcontactInfoPerson1Name(data.contactInfoPerson1Name);
      setcontactInfoPerson1Number(data.contactInfoPerson1Number);
      setcontactInfoPerson1LandNumber(data.contactInfoPerson1LandNumber);
      setcontactInfoPerson1Email(data.contactInfoPerson1Email);
      setcontactInfoPerson2Number(data.contactInfoPerson2Number);
      setcontactInfoPerson2LandNumber(data.contactInfoPerson2LandNumber);
      setcontactInfoPerson2Email(data.contactInfoPerson2Email);
      setworkedSince(data.workedSince);
      setageRange1(data.ageRange1);
      setageRange2(data.ageRange2);
      setageRange3(data.ageRange3);
      setsex(data.sex);
      setnationality(data.nationality);
      setdisability(data.disability);
      setdisabilityType(data.disabilityType);
      setdoubleDisability(data.doubleDisability);
      setemergencyCases(data.emergencyCases);
      setserviceIsFree(data.serviceIsFree);
      setservicePrice(data.servicePrice);
      setserviceDays(data.serviceDays);
      setserviceHours(data.serviceHours);
      setemergencyHours(data.emergencyHours);
      setserviceProviderPsyc(data.serviceProviderPsyc);
      setserviveTypePhsyc(data.serviveTypePhsyc);
      setserviveTypePhsycOther(data.serviveTypePhsycOther);
      setserviceProviderSec(data.serviceProviderSec);
      setserviveTypeSec(data.serviveTypeSec);
      setserviceProvidermed(data.serviceProvidermed);
      setserviveTypemed(data.serviveTypemed);
      setserviveTypemedOther(data.serviveTypemedOther);
      setterms(data.terms);
      setdocumentsRequired(data.documentsRequired);
      setbasicActions(data.basicActions);
    };
    fetchEntry();
  }, [match.id]);
  ...
  const submitHandler = async (e) => {
    ...
    try {
      const { data } = await axios.put(`/api/entries/${match.id}`, {
        inChargeName,
        institutionName,
        institutionType,
        institutionAdress,
  ...
  return (
    <div>
      <form className={classes.form} onSubmit={submitHandler}>
        ...
        />
        <label htmlFor="disability">ذوي إعاقة</label>
        <TextField
          type="text"
          id="disability"
          value={disability}
